...
if(shoot && timer <= 0 && !(ds_list_empty(bammo))){
	...
	ds_list_insert(spent, 0, bullet);
	ds_list_delete(bammo, ds_list_find_index(bammo, bullet));
}
timer -= global.dt;
/*
if(ds_list_size(bammo)>maxbees){
	for(var f = 0; f<maxbees-(ds_list_size(bammo)); f++){
		ds_list_delete(bammo, ds_list_find_index(bammo, null));
	}
}*/

//sort array of spent bees
//collect bees
if(collect && !(ds_list_empty(spent))){
	for(j = 0; j<ds_list_size(spent); j++){
